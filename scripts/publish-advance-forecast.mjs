...
import { parseRowEvents, rowsInWindow, applyRowEvents } from './lib/stall4-rows.mjs';
...
function currentActuals(model, nowIso, msRows, factorByStall, occRows) {
  ...
  for (const s of STALLS) {
    const movement = msRows.length
      ...
      : null;
    const liveEvents = rowEventsByStall[s];
    const liveRows = liveEvents ? rowsInWindow(liveEvents, nowEpoch - 15 * 60, nowEpoch + 1, { quietHours: ROW_QUIET[s] }) : null;
    out[s] = {
      actual: liveRows !== null ? liveRows : (movement ? movement.replenish : null),
