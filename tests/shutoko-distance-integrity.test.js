...
test('S17 所沢(関越)→霞ヶ関 viaGaikan 物理距離 ≈ 32-34 km', () => {
  const data = loadAll();
  const find = (id) => data.ics.find((x) => x.id === id);
  ...
  // 関越 9.4 + 外環 7.8 + 首都高 17 = 34.2
  within(r.totals.distanceKmOneway, 33.0, '所沢→霞ヶ関 viaGaikan');
});

test('S18 柏(常磐)→霞ヶ関 viaGaikan 物理距離 ≈ 41-43 km (常磐10.8 + 外環14 + 首都高17)', () => {
  const data = loadAll();
  const find = (id) => data.ics.find((x) => x.id === id);
  ...
  within(r.totals.distanceKmOneway, 42.0, '柏→霞ヶ関 viaGaikan');
});
