...
test('S4 S1川口線: 川口JCT→霞ヶ関 ≈ 32 km (S1 12.3 + C2 7.5 + 5号 8.1 + C1 4.1)', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'kawaguchi_jct', 'kasumigaseki');
  within(km, 32.0, 'kawaguchi_jct → kasumigaseki (S1→C2→5号→C1)');
});

test('S5 6号三郷線: 三郷JCT→霞ヶ関 ≈ 26 km (6号→C2→5号→C1)', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'misato_jct', 'kasumigaseki');
  within(km, 26.0, 'misato_jct → kasumigaseki');
});

test('S6 7号小松川線: 篠崎→霞ヶ関 ≈ 17-18 km (7号 10.4 + 6号向島 1.4 + C1 5.8)', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'shinozaki', 'kasumigaseki');
  within(km, 17.5, 'shinozaki → kasumigaseki (7号→6号向島→C1)');
});

test('S7 湾岸→9号→C1: 湾岸市川→霞ヶ関 ≈ 26 km (高谷JCT 9.8 + 9号 10.5 + C1 5.8)', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'wangan_ichikawa', 'kasumigaseki');
  within(km, 26.0, 'wangan_ichikawa → kasumigaseki');
});

// ── 主要 baseline → 空港中央 の検証 ──

test('S8 用賀→空港中央 (3号→C2→大井JCT→湾岸 default) ≈ 24 km', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'tokyo_ic', 'kukou_chuou');
  within(km, 23.7, 'tokyo_ic → kukou_chuou');
});

test('S9 用賀→湾岸環八 (default 3号→C2→大井JCT→湾岸) ≈ 26 km', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'tokyo_ic', 'wangan_kanpachi');
  within(km, 26.4, 'tokyo_ic → wangan_kanpachi');
});

test('S10 三郷→空港中央 (default 6号→C2→大井JCT→湾岸) ≈ 40 km', () => {
  const data = loadAll();
  const km = shutokoKmTo(data, 'misato_jct', 'kukou_chuou');
  within(km, 40.4, 'misato_jct → kukou_chuou');
});
...
test('S17 所沢(関越)→霞ヶ関 viaGaikan 物理距離 ≈ 42 km', () => {
  const data = loadAll();
  const find = (id) => data.ics.find((x) => x.id === id);
  ...
  // 関越(8.6=9.4-大泉JCT0.8) + 外環(7.8) + 首都高 bijogi_jct→kasumigaseki(25.9) = 42.3km
  within(r.totals.distanceKmOneway, 42.3, '所沢→霞ヶ関 viaGaikan');
});
...
test('S18 柏(常磐)→霞ヶ関 viaGaikan 物理距離 ≈ 56 km (柏10.8 + 外環14 + S1経由32)', () => {
  const data = loadAll();
  const find = (id) => data.ics.find((x) => x.id === id);
  ...
  // 常磐(10.8) + 外環(14.0) + 首都高 kawaguchi_jct→kasumigaseki(32.0) = 56.8km
  within(r.totals.distanceKmOneway, 56.8, '柏→霞ヶ関 viaGaikan');
});
