...
test('binMovementCounts: 占有変化もraw変化も弱い候補は除外する', () => {
  ...
  const movement = binMovementCounts(rows, ['stall3'], {
    absThreshold: 6,
    debounceSec: 120,
    occRows,
    weakRawDelta: 8,
  ...
  assert.equal(movement.replenish.stall3 || 0, 0);
  assert.equal(movement.departure.stall3 || 0, 0);
});

test('binMovementCounts: 夜は占有変化とraw変化が強く逆向きの候補を除外する', () => {
  ...
  const movement = binMovementCounts(rows, ['stall1'], {
    absThreshold: 8,
    debounceSec: 120,
    occRows,
    weakRawDelta: 8,
  ...
  assert.equal(movement.replenish.stall1 || 0, 0);
  assert.equal(movement.departure.stall1 || 0, 0);
});
