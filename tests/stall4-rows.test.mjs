...
test("rowsInWindow: ビン内の列移動の回数(rows は数えない。unit:'rows' で列数)", () => {
  ...
  assert.strictEqual(rowsInWindow(ev, start + 900, start + 1800), 1);         // 12:00-12:15 (2列移動でも1回)
  assert.strictEqual(rowsInWindow(ev, start + 900, start + 1800, { unit: "rows" }), 2);
...
test("applyStall4Rows: stall4 を列移動由来に置き換え、他の乗り場は触らない", () => {
  ...
  assert.deepStrictEqual(out.stalls, { stall3: 2, stall4: 1 });
...
test("applyRowEvents: 3号と4号を同時に置き換え、null の乗り場は従来のまま", () => {
  ...
  assert.deepStrictEqual(applyRowEvents(bin, { stall3: ev3, stall4: ev }).stalls, { stall2: 1, stall3: 1, stall4: 1 });
  assert.deepStrictEqual(applyRowEvents(bin, { stall3: null, stall4: ev }).stalls, { stall2: 1, stall3: 9, stall4: 1 });
});
