...
test('P3 関越 所沢→霞ヶ関 viaGaikan 総距離 ≈ 42km (8.6 + 7.8 + 美女木→霞ヶ関25.9)', () => {
  const data = loadAll();
  const entry = { ...find(data, 'tokorozawa'), _viaGaikan: true };
  const r = judgeRoute({
    outerRoute: 'kanetsu', entryIc: entry, exitIc: find(data, 'kasumigaseki'), roundTrip: false,
  }, data);
  // 関越(8.6) + 外環(7.8) + 首都高 bijogi_jct→kasumigaseki(25.9) = 42.3km
  // viaGaikan時の首都高 entry は美女木JCT (5号池袋線端点)
  assert.ok(r.totals.distanceKmOneway >= 40.0 && r.totals.distanceKmOneway <= 45.0,
    `総距離 期待40-45km、実測 ${r.totals.distanceKmOneway}km`);
});
