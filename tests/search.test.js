...
test('A1: aliases 無し・1方面 → 単一の "${name}" 値', () => {
  const groups = [{
    id: 'tomei', label: '東名',
    ics: [{ ic: { id: 'tokyo_ic', name: '東京IC' } }],
  }];
  const out = buildSearchEntries(groups);
  assert.deepEqual(out, [{ value: '東京IC', icId: 'tokyo_ic' }]);
});
...
test('A2: aliases 1件 → 「／alias」インライン挿入', () => {
  ...
  const out = buildSearchEntries(groups);
  assert.deepEqual(out, [{ value: '新空港IC／成田', icId: 'shin_kukou' }]);
});
...
test('A3: aliases 複数 → 「・」区切りで全部インライン', () => {
  ...
  const out = buildSearchEntries(groups);
  assert.deepEqual(out, [{ value: '新空港IC／成田空港・成田', icId: 'shin_kukou' }]);
});
...
test('A5: 同 IC が複数方面 → 方面ごとに entry', () => {
  ...
  assert.ok(values.includes('成田空港IC'));
...
test('B8: 全 entry の value → icId が Map で逆引き可能', () => {
  const entries = [
    { value: '東京IC', icId: 'tokyo_ic' },
    { value: '新空港IC／成田', icId: 'shin_kukou' },
  ];
  const map = buildValueToIcIdMap(entries);
  assert.equal(map.get('東京IC'), 'tokyo_ic');
  assert.equal(map.get('新空港IC／成田'), 'shin_kukou');
  assert.equal(map.get('存在しない'), undefined);
});

test('B9: 同 ic_id が複数 value から引けても OK', () => {
  const entries = [
    { value: '成田空港IC', icId: 'narita_kukou' },
    { value: '成田空港IC', icId: 'narita_kukou' },
  ];
  const map = buildValueToIcIdMap(entries);
  assert.equal(map.get('成田空港IC'), 'narita_kukou');
});

// --- C. 統合・実データ整合 ---

test('C10: 検索 value は同じ値で異なる icId を指さない（実データの全 entry で衝突なし）', async () => {
  ...
  const conflicts = [...valueToIcIds.entries()].filter(([, set]) => set.size > 1);
  assert.deepEqual(conflicts, [], '同じ値で異なる icId の衝突があります: ' + JSON.stringify(conflicts));
});
