...
test('D8 新規 湾岸環八 alt: 横浜青葉→湾岸環八 → C2_wangan default 17.2km', () => {
  ...
  const shutokoSeg = r.segments.find((s) => s.route === 'shutoko');
  assert.equal(shutokoSeg.distanceKm, 26.4, 'tokyo_ic→wangan_kanpachi default C2 km');
});
...
test('D14 浮島JCT→湾岸環八 直行 (default 7.7km)', () => {
  const data = loadAll();
  const r = run({
    outerRoute: 'aqua',
    entryIc: findIc(data, 'kisarazu_kaneda'),
  ...
  const shutokoSeg = r.segments.find((s) => s.route === 'shutoko');
  assert.equal(shutokoSeg.distanceKm, 7.7, 'ukishima_jct→wangan_kanpachi 湾岸直行');
});
