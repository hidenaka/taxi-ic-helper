...
- [ ] **Step 2.3: `pax-estimator.mjs` の `estimatePax` を改修**
...
- [ ] **Step 3.2: テスト実行 → 失敗確認 (signature ミスマッチ)**
...
- [ ] **Step 3.3: `arrival-transformer.mjs` を改修**
...
- [ ] **Step 4.1: `fetch-arrivals.mjs` で master を読み込み、transformArrivals に渡す**
...
- [ ] **Step 4.2: `generate-mock-arrivals.mjs` で同様に master を読み込み、transformArrivals に渡す**
...
- [ ] **Step 4.4: コミット (master ファイル未作成のため runtime 検証は Task 5/6 後)**
...
- [ ] **Step 5.1: サブエージェントに 47 便の機材調査を依頼**
...
- [ ] **Step 6.1: 33 路線の典型機材を割り当て、JSON ファイルを作成**
...
- [ ] **Step 7.4: unknownAircraft / estimatedPax null 件数を計測**
...
- [ ] **Step 7.5: ローカルの実データを破棄 (mock に戻す)**
...
- [ ] `npm test` 全件パス (289 件)
- [ ] mock 生成エラーなし
- [ ] 実 ODPT データで `estimatedPax=null` の便が 0〜2 件 (改修前 46〜47 件)
- [ ] ANA 国際線 47 便すべてで `estimatedPax` が非 null
- [ ] 本番 URL で機材不明件数が大幅減
- [ ] `data/aircraft-by-flight-number.json` の `_meta.source` に出典 URL 明記
