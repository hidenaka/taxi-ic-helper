...
2026-05-11 から開始した観測ジョブの中間スナップショット (jsonl 862 行、ts 範囲 2026-05-10 13:31:19+09:00 〜 2026-05-15 07:37:29+09:00) を解析した。信頼サブセット 391 行 (schema=3 ∧ luminance>=30 ∧ ts 順序正常 ∧ stalls 非 null) を基準として、5 分刻み出庫量・H6/H8/H9 の暫定傾向、5/31 本分析・ROI v4 設計への提言をまとめる。
...
- 5 分刻み出庫: 信頼サブセット 391 ticks で T1 合計 185 台 / T2 合計 151 台 / 全体 336 台 — 04 時に集団出庫イベント (5 分で 25-29 台)、3 日連続再現
...
- v3 行: 727
- 信頼サブセット (schema=3 ∧ luminance>=30 ∧ ts 順序正常 ∧ stalls 非 null): 391

![stall 別 occupied ヒートマップ](figures/2026-05-14/03-stall-occupancy-heatmap.png)

## 3. 5 分刻み出庫量の観測 (Phase A 観測の核心アウトプット)

信頼サブセット (391 ticks、5 分刻み) で計測した出庫:

- T1 (stall1+2、JAL/T1 側) 合計: **185 台**
...
|   7.00 |          0.19 |          0.00 |             0.19 |         6.00 |         0.00 |     32.00 |           143.92 |
...
|   7.00 |             6.00 |             0.00 |                6.00 |             236.53 | 32.00 |
...
- T1 (stall1+2) Pearson r = -0.231
- T2 (stall3+4) Pearson r = -0.395
- 信頼サブセット n = 391
...
5 分均等グリッド (742 rows) で `arrivals_window.estimated_taxi_pax_sum` (signal) と outflow (target) の Pearson 相関を ±60 分でスイープ:
...
- occupied_estimate Pearson r = 0.374 (n=391)
- diff_occupied_from_prev Pearson r = 0.411 (n=390)
...
| 指標 | 夜間 | 昼間 |
|---|---|---|
| luminance_std | n=336, mean=18.503, median=23.165, std=10.076 | n=384, mean=42.021, median=44.315, std=9.185 |
| edge_density | n=336, mean=0.073, median=0.084, std=0.060 | n=384, mean=0.249, median=0.282, std=0.095 |
...
**メタ情報**: 分析スクリプト `docs/research/scripts/phase-a-mid-analysis.py` を実行 (2026-05-15 07:37:29+09:00 時点の jsonl)。再現性のため commit 済み。venv は `~/.venvs/taxi-ic-phase-a/`。
