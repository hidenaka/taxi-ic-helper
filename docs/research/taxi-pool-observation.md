...
**実運用上の終了日**: 未設定（手動停止まで継続）。`scripts/observe-tick-local.sh` / `scripts/track_vehicles.py` の `STOP_DATE` は実質無期限の `2099-01-01` に設定済み。停止は手動 `./scripts/install-observe-launchd.sh uninstall`、または `STOP_DATE` を任意の日付に再設定する。
